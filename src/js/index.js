...
(function () {
  class Canvas {
    ...
    renderFall() {
      ...
      switch (this.fallType) {
        case "snow":
          ...
          break;
        case "heart":
          this.time % config.heartInterval == 0 &&
            this.fallDots.push(new Heart(config.heart));
          break;
        case "mix":
          this.time % config.snowInterval == 0 &&
            this.fallDots.push(new Snowflake(config.snow));
    ...
    renderDialogue() {
      ...
      ctx.font =
        this.dialogueOpt["font" + this.dialogue.type] || this.dialogueOpt.font1;
    ...
    sunset() {
      ...
      this.fireworkCtx.fillStyle = `hsla(210, 60%, 5%, ${
        0.01 * (20 - 20 * (this.sunsetTime / config.sunset))
      })`;
    ...
    renderFireworks() {
      ...
      for (let i = this.fireworks.length - 1; i >= 0; --i) {
        this.skyColor =
          this.skyColor.lightness >= this.fireworks[i].getSkyColor().lightness
            ? this.skyColor
    ...
    getDotsPostion(wordsArr, showFireworks) {
      ...
      words.forEach((item, index) => {
        ...
        length % 2 == 0
          ? (x =
              config.width / 2 + (index - length / 2) * size + (1 / 2) * size)
          : (x = config.width / 2 + (index - Math.floor(length / 2)) * size);
        ...
        showFireworks &&
          this.fireworks.push(
            new Firework({ wait: 30, radius: 2, x, yEnd: y, dots, prtOption })
          );
      });
